    ...
    reader.onload = (e) => {
      ...
      for (let i = 0; i < header.clrUsed * 4; i += 4) {
        ...
        const avg = Math.round(
          palette[i + 2] * 0.299 + palette[i + 1] * 0.587 + palette[i] * 0.114
        );
...
export const processBmpWithBorder = async (file: File): Promise<void> => {
  ...
  return new Promise((resolve, reject) => {
    reader.onload = (e) => {
      ...
      // const originalPadding = (4 - (originalRowSize % 4)) % 4; // Removed unused variable
      ...
      for (let y = 0; y < originalHeight; y++) {
        ...
        for (let x = 0; x < originalWidth; x++) {
          for (let b = 0; b < bytesPerPixel; b++) {
            newData[newPos + b] =
              originalData[originalRowStart + x * bytesPerPixel + b];
          }
          newPos += bytesPerPixel;
        }
  ...
  return new Promise((resolve, reject) => {
    reader.onload = (e) => {
      ...
      if (
        ...
        reject(
          new Error(
            "Неподдерживаемый формат BMP! Требуется 24-битный TrueColor BMP без сжатия."
          )
        );
        return;
      }
      ...
      const originalRowSize =
        Math.floor((header.bpp * originalWidth + 31) / 32) * 4;
